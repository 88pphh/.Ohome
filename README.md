...
**④ 서버 위치를 서울로 (한국에서 쓴다면 꼭)**

Vercel은 기본값이 **미국 동부**라, 그냥 두면 페이지를 열 때마다 태평양을 왕복합니다.

**Settings → Functions → Function Region → `Seoul (icn1)` → [Save]**

바꾼 뒤 **Deployments → 맨 위 배포의 ⋯ → [Redeploy]** 를 눌러야 적용됩니다. 30초면 끝나고, 체감이 확실히 달라집니다.
